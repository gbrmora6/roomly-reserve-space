-- Atualizar função get_room_availability para ignorar itens apenas no carrinho
...
BEGIN
    ...
    WHILE current_hour < end_hour LOOP
        ...
        -- Verificar se há reserva confirmada neste horário (APENAS paid e in_process de reservas reais)
        SELECT EXISTS(
        ...
        -- Horário está disponível (não verificamos mais carrinho, apenas reservas reais)
        RETURN QUERY SELECT 
            (current_hour || ':00'), 
            TRUE, 
            NULL::TEXT;
...
-- Atualizar função get_equipment_availability para ignorar itens apenas no carrinho
...
BEGIN
  ...
  WHILE current_hour < end_hour LOOP
    ...
    -- Verificar quantas unidades estão reservadas neste horário (APENAS paid e in_process de reservas reais)
    ...
    -- Calcular quantidade disponível (não consideramos mais carrinho, apenas reservas reais)
    DECLARE
