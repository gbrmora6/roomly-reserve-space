-- Corrigir função get_room_availability para garantir que funciona corretamente
...
BEGIN
  ...
  -- Se não há schedule para este dia, verificar horários padrão da sala
  IF NOT FOUND THEN
    SELECT r.open_time AS start_time, r.close_time AS end_time INTO room_schedule
    FROM rooms r 
    WHERE r.id = p_room_id;
    
    -- Se não tem horários definidos, retorna vazio (sala fechada)
    IF room_schedule.start_time IS NULL OR room_schedule.end_time IS NULL THEN
      RETURN;
    END IF;
  END IF;
