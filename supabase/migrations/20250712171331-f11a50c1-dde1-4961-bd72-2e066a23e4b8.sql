-- Criar função get_equipment_availability similar à get_room_availability
...
BEGIN
  ...
  SELECT e.quantity, e.open_time, e.close_time INTO equipment_info
  FROM equipment e
  ...
  -- Buscar horários de funcionamento do equipamento para o dia da semana
  ...
  -- Se não há schedule para este dia, verificar horários padrão do equipamento
  IF NOT FOUND THEN
    SELECT equipment_info.open_time AS start_time, equipment_info.close_time AS end_time INTO equipment_schedule;
    
    -- Se não tem horários definidos, retorna vazio (equipamento fechado)
    IF equipment_schedule.start_time IS NULL OR equipment_schedule.end_time IS NULL THEN
      RETURN;
    END IF;
  END IF;
  ...
  WHILE current_hour < EXTRACT(HOUR FROM equipment_schedule.end_time) LOOP
    ...
    SELECT COALESCE(SUM(be.quantity), 0) INTO booking_record
    FROM cart_items ci
    ...
    reserved_quantity := reserved_quantity + COALESCE(booking_record, 0);
